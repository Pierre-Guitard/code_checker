...
const githubController = {
  getRepositories: async (req, res) => {
    try {
      ...
      const totalFiles = Array.isArray(contentsRes.data)
        ? contentsRes.data.length
        : 0;
  ...
  getAllCommitsFromUser: async (req, res) => {
    ...
    try {
      ...
      const response = await axios.get(
        "https://api.github.com/repos/Pierre-Guitard/code_checker/commits?author=H4SS4NN",
        header
      );
      ...
      await Promise.all(
        urlArray.map(async (url) => {
          ...
          files.forEach((file) => {
            ...
            const shouldIgnore = ignoredFiles.some((ignored) =>
      ...
      const codeReview = await axios.post(
        "http://localhost:3000/api/groq/review",
        commitsContent
      );
